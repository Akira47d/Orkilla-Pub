...
client.on("ready", () => {
    ...
    client.user.setActivity("383 membres | 👀", {type: 3})
})
...
client.on('guildMemberAdd', function (member) {
    ...
    member.guild.channels.get('604769370304151555').send(embed)
})
...
client.on('guildMemberRemove', function (member) {
    let embed = new Discord.RichEmbed()
        ...
        .setFooter('Nous sommes désormais ' + member.guild.memberCount)
    member.guild.channels.get('604769372191588372').send(embed)
})
...
client.on("message", function (message) {
    ...
    /*Info warn*/
    if (args[0].toLowerCase() === prefix + "inwarns") {
        if (!message.member.hasPermission('MANAGE_MESSAGES')) return message.channel.send("Vous n'avez pas la permission d'utiliser cette commande")
        let member = message.mentions.members.first()
        if (!member) return message.channel.send("Veuillez mentionner un membre")
        let embed = new Discord.RichEmbed()
...
client.on('message', function (message) {
    ...
    if (args[0].toLowerCase() === prefix + "8ball") {
        ...
        let answers = ["Non :x:", "J'ai envie de dormir :zzz:", "Balec :face_palm:", "Peut être... :thinking:", "Absolument :interrobang:", "Va en enfer :fire: ", "Laisse moi tranquille :nauseated_face: ", "Carrément :wink: ", "Je suppose :stuck_out_tongue_winking_eye: ", "Oui :white_check_mark: ", "J'ignore :eyes: "]
